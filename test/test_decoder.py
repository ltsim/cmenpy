...
class TestStringSpace:
    def test_alphabetical_index(self):
        sp = StringDecoder(valid_sets=("relu", "tanh", "elu"))
        assert sp.labels[0].tolist() == ["elu", "relu", "tanh"]

    def test_multivar(self):
        sp = StringDecoder(valid_sets=(("relu", "tanh", "sigmoid"), ("adam", "sgd")))
        assert sp.n_vars == 2
    ...
    def test_truncation_boundary(self):
        sp = StringDecoder(valid_sets=("a", "b", "c"))
...
class TestCategoricalSpace:
    def test_mixed_types(self):
        sp = CategoricalDecoder(valid_sets=((8, 16, 32), ("l1", "l2", "none")))
        assert sp.decode(np.array([2.1, 0.9])) == [8, "l1"]

    def test_preserves_int_type(self):
        sp = CategoricalDecoder(valid_sets=((8, 16, 32),))
...
class TestPermutationSpace:
    def test_validity_arbitrary_floats(self):
        sp = PermutationDecoder(valid_set=["A", "B", "C", "D"])
    ...
    def test_generate_is_permutation(self):
        sp = PermutationDecoder(valid_set=[1, 2, 3, 4, 5])
        sp.set_seed(0)
        out = sp.generate()
    ...
    def test_rejects_singleton(self):
        with pytest.raises(ValueError):
            PermutationDecoder(valid_set=["only"])

    def test_encode_maps_to_sorted_indices(self):
        sp = PermutationDecoder(valid_set=["A", "B", "C"])
...
class TestBinarySpace:
    ...
    def test_rejects_zero_vars(self):
        with pytest.raises(ValueError):
            BinaryDecoder(n_vars=0)


class TestBoolSpace:
    def test_dtype_is_bool(self):
        sp = BoolDecoder(n_vars=4)
...
ALL_SPACES = [
    ...
    lambda: PermutationDecoder(valid_set=["a", "b", "c"]),
    lambda: BinaryDecoder(n_vars=4),
    lambda: BoolDecoder(n_vars=4),
]
